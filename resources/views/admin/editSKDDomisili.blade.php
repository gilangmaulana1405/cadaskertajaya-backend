     ...
     <div class="content-wrapper">
         <div class="row">
             <div class="col-md-8 grid-margin stretch-card">
                 <div class="card">
                     <div class="card-body">
                         <h4 class="card-title">Form Edit SKD Domisili</h4>
                         <p class="card-description">
                             Data Diri
                         </p>
                         <form class="forms-sample" method="post" action="/admin/skdDomisili/{{ $data->id }}">
                             @method('PUT')
                             @csrf
                             ...
                             <div class="form-group">
                                 <label for="inputStatusKependudukan">Status Kependudukan</label>
                                 <select class="form-control" name="status_kependudukan">
                                     <option value="Penduduk Lama" {{ old('status_kependudukan',$data->status_kependudukan) == 'Penduduk Lama' ? 'selected' : '' }}>Penduduk Lama</option>
                                     <option value="Pendatang" {{ old('status_kependudukan',$data->status_kependudukan) == 'Pendatang' ? 'selected' : '' }}>Pendatang</option>
                                 </select>
                             </div>
                             <button type="submit" class="btn btn-primary mr-2">Update</button>
                             <a href="/admin/skdDomisili" class="btn btn-light">Cancel</a>
                         </form>
                     </div>
                 </div>
             </div>
